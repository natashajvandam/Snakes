import { getInputDirection } from './input3.js'
import { snakeBody as snakeBody1, removeSnake as removeSnake1 } from './snake.js'
import { snakeBody as snakeBody2, removeSnake as removeSnake2 } from './snake.js'
import { memory } from './script.js'

export let snakeBody = [{x: 13, y: 13}];
let newSegments = 0;
...
export function snakeCrashwith1 () {
  if (onSnake (snakeBody1[0]) && snakeBody.length > 1) {
    return true;
  } else if (onSnake (snakeBody1[0]) && snakeBody.length === 1) {
    memory.push('dead');
    removeSnake();
    return false;
  } else {
